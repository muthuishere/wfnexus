...
<ctx-optimize>
  ...
  <store>MULTI-MODULE repo, pre-built knowledge store at `~/ctxoptimize/bug-fixer-platform/` — one graph per module + a navigator, 4 modules declared in `.ctxoptimize/config.json`.</store>
  ...
  federates via the navigator (`~/ctxoptimize/bug-fixer-platform/navigator.md`; `--modules all|a,b` widens).
  ...
  <no-local-store>Fresh clone with nothing at `~/ctxoptimize/bug-fixer-platform/`? Run `ctx-optimize up` —
